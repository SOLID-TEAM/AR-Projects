...
input_threshold = input('Detection threshold(0.1): ')
scale_multipler = float(input('Scale multiplier(1 or 0.5): '))
...
img_temp = cv2.resize(img_temp, (int(img_org.shape[1] * scale_multipler), int(img_org.shape[0] * scale_multipler)),  interpolation=cv2.INTER_NEAREST)
img_target = cv2.resize(img_target, (int(img_target.shape[1] * scale_multipler), int(img_target.shape[0] * scale_multipler)), interpolation=cv2.INTER_NEAREST)
...
for pt in zip(*locations[::-1]):
    cv2.rectangle(img_org, (int(pt[0] / scale_multipler), int(pt[1] / scale_multipler)), (int(pt[0] / scale_multipler + h_trg / scale_multipler), int(pt[1] / scale_multipler + w_trg / scale_multipler)), (0, 255, 0), 1)
